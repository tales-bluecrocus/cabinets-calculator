...
define('CABINETS_CALC_VERSION', '0.0.4');
...
class CabinetsCalculator
{
	...
	/**
	 * Handle quote submission and send via Postmark
	 */
	public function handle_quote_submission($request)
	{
		...
		if (empty($postmark_api_key)) {
			// Log the submission even without Postmark
	...
	private function send_postmark_email($data, $api_key, $admin_email, $from_email)
	{
		...
		$html_body = $this->build_email_html($data);
		$text_body = $this->build_email_text($data);
		...
		if ($response_code !== 200) {
			error_log('Postmark API Error: ' . wp_remote_retrieve_body($response));
			return false;
		}
	...
	private function send_customer_confirmation($data, $api_key, $from_email)
	{
		...
		$html_body = $this->build_customer_email_html($data);
		$text_body = $this->build_customer_email_text($data);
...
?>
				...
				<div class="section">
					...
					<table>
						<tr>
							...
							<td class="value"><?php echo esc_html($layout_labels[$data['layoutType']] ?? $data['layoutType']); ?></td>
						</tr>
						<tr>
							<td class="label">Linear Feet:</td>
							<td class="value"><?php echo esc_html($data['linearFeet']); ?> ft</td>
						</tr>
						<tr>
							<td class="label">Ceiling Configuration:</td>
							<td class="value"><?php echo esc_html($ceiling_labels[$data['ceilingConfig']] ?? $data['ceilingConfig']); ?></td>
						</tr>
						<?php if ($data['hasIsland'] && !empty($data['islandDimensions'])): ?>
							<tr>
								<td class="label">Island:</td>
								<td class="value"><?php echo esc_html($data['islandDimensions']['length'] . 'ft × ' . $data['islandDimensions']['width'] . 'ft'); ?></td>
							</tr>
						<?php endif; ?>
					</table>
				</div>
				...
				<div class="estimate-box">
					...
					<table>
						<tr>
							...
							<td style="text-align: right;">$<?php echo number_format($estimate['cabinet']['subtotalLow']); ?> - $<?php echo number_format($estimate['cabinet']['subtotalHigh']); ?></td>
						</tr>
						...
						<tr style="border-top: 2px solid #1a365d;">
							...
							<td style="text-align: right;" class="total">$<?php echo number_format($estimate['total']['low']); ?> - $<?php echo number_format($estimate['total']['high']); ?></td>
						</tr>
					</table>
				</div>
	...
	/**
	 * Build plain text email for admin
	 */
	private function build_email_text($data)
	{
		...
		if ($data['hasIsland'] && !empty($data['islandDimensions'])) {
	...
	?>
		...
		<body>
			<div class="container">
				<div class="header">
					<h1>BluePrint Cabinets</h1>
					<p>Your Kitchen Estimate</p>
				</div>
				...
				<div class="content">
					...
					<div class="estimate-box">
						...
						<table style="width: 100%; margin: 20px 0;">
							...
							<?php endif; ?>
							<tr style="border-top: 2px solid #1a365d; font-size: 20px;">
								<td><strong>TOTAL ESTIMATE</strong></td>
								<td style="text-align: right;"><strong>$<?php echo number_format($estimate['total']['low']); ?> - $<?php echo number_format($estimate['total']['high']); ?></strong></td>
							</tr>
						</table>
					</div>

					<h3>What's Next?</h3>
					<p>Our design team has received your request and will reach out within 24 hours to discuss your project in detail. During our free design consultation, we'll:</p>
					<ul>
						<li>Review your specific kitchen layout</li>
						<li>Discuss finish and style options</li>
						<li>Provide a precise quote</li>
						<li>Answer any questions you have</li>
					</ul>

					<p style="text-align: center;">
						<a href="https://blueprintcabinets.com/showroom" class="cta">Visit Our Showroom</a>
					</p>
				</div>
	...
	private function build_customer_email_text($data)
	{
		...
		$text .= "WHAT'S NEXT?\n";
		$text .= "Our design team will reach out within 24 hours.\n\n";
	...
	private function log_submission($data)
	{
		...
		$wpdb->insert(
			...
			[
				...
				'has_island' => $data['hasIsland'] ? 1 : 0,
				'island_dimensions' => $data['hasIsland'] ? json_encode($data['islandDimensions']) : null,
	...
	?>
		<div class="wrap">
			...
			<form method="post" action="options.php">
				...
				<h2>Postmark Email Settings</h2>
				<table class="form-table">
					<tr>
						<th scope="row"><label for="cabinets_calc_postmark_api_key">Postmark API Key</label></th>
						<td>
							<input type="password" name="cabinets_calc_postmark_api_key" id="cabinets_calc_postmark_api_key"
								value="<?php echo esc_attr(get_option('cabinets_calc_postmark_api_key')); ?>" class="regular-text">
					...
					<tr>
						...
						<td>
							<input type="email" name="cabinets_calc_admin_email" id="cabinets_calc_admin_email"
								value="<?php echo esc_attr(get_option('cabinets_calc_admin_email', get_option('admin_email'))); ?>" class="regular-text">
					...
					<tr>
						...
						<td>
							<input type="email" name="cabinets_calc_from_email" id="cabinets_calc_from_email"
								value="<?php echo esc_attr(get_option('cabinets_calc_from_email')); ?>" class="regular-text">
				...
				<table class="form-table">
					<tr>
						...
						<td>
							<input type="text" name="cabinets_calc_company_name" id="cabinets_calc_company_name"
								value="<?php echo esc_attr(get_option('cabinets_calc_company_name', 'BluePrint Cabinets')); ?>" class="regular-text">
						</td>
					</tr>
					<tr>
						<th scope="row"><label for="cabinets_calc_showroom_url">Showroom URL</label></th>
						<td>
							<input type="url" name="cabinets_calc_showroom_url" id="cabinets_calc_showroom_url"
								value="<?php echo esc_attr(get_option('cabinets_calc_showroom_url')); ?>" class="regular-text">
						</td>
					</tr>
					<tr>
						<th scope="row"><label for="cabinets_calc_contact_phone">Contact Phone</label></th>
						<td>
							<input type="text" name="cabinets_calc_contact_phone" id="cabinets_calc_contact_phone"
								value="<?php echo esc_attr(get_option('cabinets_calc_contact_phone')); ?>" class="regular-text">
						</td>
					</tr>
				</table>
		...
		<div class="wrap">
			...
			<table class="wp-list-table widefat fixed striped">
				...
				<tbody>
					<?php if (empty($quotes)): ?>
						<tr>
							<td colspan="9">No quotes submitted yet.</td>
						</tr>
					<?php else: ?>
...
<?php
	...
	/**
	 * Check if calculator should be loaded
	 */
