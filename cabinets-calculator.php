...
// Core constants
define('CABINETS_CALC_VERSION', '0.1.0');
define('CABINETS_CALC_PLUGIN_DIR', plugin_dir_path(__FILE__));
define('CABINETS_CALC_PLUGIN_URL', plugin_dir_url(__FILE__));

// Autoload Composer dependencies
$autoload_path = CABINETS_CALC_PLUGIN_DIR . 'vendor/autoload.php';
if (file_exists($autoload_path)) {
	require_once $autoload_path;
} else {
	add_action('admin_notices', function () {
		echo '<div class="notice notice-error"><p><strong>Cabinets Calculator:</strong> Composer dependencies not installed. Run <code>composer install</code>.</p></div>';
	});
	return;
}

// Load email service
require_once CABINETS_CALC_PLUGIN_DIR . 'backend/services/EmailService.php';
...
class CabinetsCalculator
{
	...
	public function handle_quote_submission($request)
	{
		...
		$email_service = new \BluePrintCabinets\CabinetsCalculator\Services\EmailService();

		// If email service is not configured, just return success
		...
		$admin_sent = $email_service->send_admin_notification($data);
		// Customer confirmation email disabled - only admin receives notification
		// $customer_sent = $email_service->send_customer_confirmation($data);
				...
				<div class="estimate-box">
					...
					<table>
						...
						<tr style="border-top:2px solid #1a365d;">
							...
							<td class="total" style="text-align:right;">$<?php echo number_format($estimate['total']['low']); ?> - $<?php echo number_format($estimate['total']['high']); ?></td>
						</tr>
					</table>
				</div>
	...
	private function log_submission($data)
	{
		...
		$wpdb->insert(
			...
			[
				...
				'estimate_high' => $data['estimate']['total']['low'],
	...
	public function enqueue_frontend_assets()
	{
		...
		if (file_exists($asset_file)) {
			...
			wp_localize_script('cabinets-calculator', 'cabinetsCalculatorConfig', [
				...
				'pluginUrl' => CABINETS_CALC_PLUGIN_URL,
				'assetsUrl' => CABINETS_CALC_PLUGIN_URL . 'frontend/dist/assets/',
			]);
		}
